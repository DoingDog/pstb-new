...
* 2026-09-23：用户将唯一生产 custom domain 改为 `n.awsl.app`，要求移除旧 `b-new.awsl.app`；部署完成后删除旧 `aioapi.js`，检查拟公开代码与历史中的密钥，再以 Unlicense 创建公开的 `DoingDog/pstb-new` 并推送。本文和实施计划中此前的域名、保留 `aioapi.js` 与不推送要求由本条覆盖。
