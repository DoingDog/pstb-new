...
* 2026-09-14：同步主规格 R3-01..R3-02 修订：derived-surface Retry发布前核对 local/source/display/host/parent/retry generations；terminal view-once response在 initial terminal commit恰好一次 settle originating action，后续 source choice不改写原 action。
...
Consumed page是独立 local-only composition。Sidebar不实例化 ordinary server destinations；只列 View与 copy、wrap、raw/source toggle、safe preview、exact UTF-8 download、top-level Blob HTML和 create-new local actions。autosync、explicit Reload或 content Reconcile完整取得并严格验证 `viewOnce:true` 200时，即使 ordinary sync已被 edit、mutation或 active deadline retire，也先捕获 transition时的 current local exact draft；pending Reload/Reconcile另捕获只含 `{actionKey,actionAttempt,startedAt}`的 `terminalOriginSettleContext`，不得包含 password、body、source、ID或 protected URL。随后移除 ordinary controls/URLs/status hooks并 dispose server controllers；aborted、truncated或 malformed response不显示 consumed。current definitely-newer autosync或仍有效的 definitely-newer Reload可在 disposal后用独立 `terminalLocalToken` auto-select response source；exact-equal保留 current source，retired或 ambiguous/older response默认显示 current local source，并在两者不同时提供 Use consumed response与 Keep current。initial terminal display/capability commit必须恰好一次 settle captured origin：Content Reconcile保留 current display时以 `content-reconcile-terminal-current-kept` succeeded；Reload显示 definitely-newer response时以 `reload-terminal-response-displayed` succeeded，staging失败而保留 current时以 `reload-terminal-response-display-failed` failed，exact-equal时以 `reload-terminal-current-unchanged` succeeded，ambiguous/older保留 current并显示 choice时以 `reload-terminal-current-kept-choice` succeeded；各 message均本地化，`settledAt`都是该 commit instant。Keep current只 dismiss且不 resettle；Use consumed response通过 offscreen/clear-before-stage transaction更新全部 derived surfaces，并只以自己的 `use-consumed-response` action在 display commit用 `use-consumed-response-displayed` settle succeeded，或在 failure用 `use-consumed-response-display-failed` settle failed，不能改写 Reload/Reconcile。两种选择都不能恢复 server action或发送 business request；background autosync不写 Last action。Consumed是允许直接显示的 document state；其 distributed limitation只在相邻 HelpTrigger。
...
state change通过一个 polite live region增量宣布。视觉状态使用文字加 icon，再辅以颜色。只有主规格 closed `ActionKey`中的 request或 fallible browser operation同时更新 originating Button和 Last action；结果保持到下一次同 key或明确 page transition，不在数秒后消失。`content-reconcile`在其 unconditional GET dispatch时 pending，在 applied/not-applied/conflict proof或 handled failure时 settle；valid terminal response改在 initial terminal display/capability commit以自己的 `settledAt`和 localized terminal message恰好一次 settle。`reload-server`的 ordinary response在 staged display commit后 success；terminal definitely-newer display、exact-equal retention和 ambiguous/older initial current display在同一 initial terminal commit按主规格 outcome table settle，target staging failure为 failed。产生该 response的 initial terminal transition不得执行 page-transition reset，settled outcome保持到下一 action或之后的明确 page transition，不能留下 pending。Keep current只 dismiss choice，既不生成 action也不 reset/resettle origin；后续 Use consumed response只更新自己的 `use-consumed-response` attempt。`use-remote`和每个 `use-consumed-response` attempt仍只有 staged display commit后 success，guard rejection、invalidation、cleanup或 fallback failure使用实际 settle time。HelpTrigger、Sidebar、mode/tab、locale/theme、password reveal、wrap、raw/source及其他 pure state toggles和无法在 origin document观察结果的 navigation不产生 pending/success/failure。delete 204在清除 sensitive references并 `history.replaceState`到 `/`后，把 query-independent success保留在 root Last action；403/409保留 ordinary records，404/delete-uncertain显示各自 terminal failure；refresh不 replay。
...
5. `FE07`：Autosave、Autosync、Network、Last action各自常驻；fresh clean/waiting/idle无 fabricated timestamp，后续 state选择主规格规定的 event time，failure不显示旧 success time；ordinary Reload、Use remote与 consumed local apply直到 staged commit才显示 success，valid terminal Reload/Reconcile在 initial terminal commit按 exact outcome显示自己的 `settledAt`，Delete各 status显示对应 transition。
6. `FE08`：closed ActionKey operation的 Button与 status同步显示 persistent outcome；union含 `content-reconcile`与 `use-consumed-response`，guard/cleanup/fallback failure可观察；terminal origin在 controller disposal后仍恰好一次 settle且无 pending，Keep current不 resettle，Use consumed response只写自己的 action；pure toggles与不可观测 navigation不改 Last action；operation feedback没有 Dialog、toast、snackbar、`alert()`或额外 status request。
