...
|---|---|---|
| Chrome | `google-chrome-versionhistory-win-stable`；`https://versionhistory.googleapis.com/v1/chrome/platforms/win/channels/stable/versions/all/releases?filter=fraction%3D1&order_by=starttime%20desc&page_size=1000` | Follow eachnonempty unique`nextPageToken` withthe exactsame query plusURL-encoded`page_token`。An exactJSON`400 INVALID_ARGUMENT` continuation retries theidentical URL atmostsix times withinthesame operation deadline；onlywhenall sixattempts matchandthe immediately preceding successful page has`releases.length < 1000` isthat trailing token treatedasvendor-stale terminal state。Initial failure、anyother status/body、six failures aftera1,000-record page andsuccessful repeated token exitnonzero。Failed bodies neverenterresponse count/hash。Acceptonly`fraction===1` records withstring`name`、numeric dotted`version` andGoogle Timestamp JSON`serving.startTime` inUTC`Z` withno fraction orexactly3/6/9 fractional digits。ValidateGregorian fields；right-pad thefraction to9digits forsource-ID conflict andtrue earliest comparison，then truncate tothree digits forcanonical artifact`releasedAt`。Group same`version`，sort/deduplicate its`name` values into`sourceReleaseIds`，anduse theearliest start instant as`releasedAt`。Conflicting repeats exitnonzero。 |
...
Tests coverTabs arrows/Home/End/normalTab、Tooltip hover/focus/click/touch/Escape/outside、Dialog/Sheet focus/Escape/return、44px targets、320px `scrollWidth===clientWidth` andfull editor width、200% zoom、reduced motion computed durations0、diff non-color prefixes、en/zh switch、system/light/dark/noStorage。Network graph：initial noCrepe/micromark/diff；visual loadsatleast oneCrepe asset andno path outside theCrepe closure；client preview loadsatleast oneMarkdown asset andno Crepe-exclusive path；diff loadsworker only。A chunk present inboth generated Crepe andMarkdown closures isallowed for either owning surface andmustnotbe misclassified bysubtracting oneclosure fromtheother。Closed-help visible-text scan rejectsallbanned prose/sample copy。Credential scan onlyallowscurrent location、representation href andexplicit clipboard result。No status-only requests。
